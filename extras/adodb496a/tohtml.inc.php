...
//JOGS agrege el ultimo parametro 
	...
	while (!$rs->EOF) {
		...
		for ($i=0; $i < $ncols; $i++) {
			...
			if($i==0 && $check!='' ){
				...
				GLOBAL $pcheck;
				if(!$pcheck) $pcheck="frm.php";
				if($check=='c')$s .=$TD . '<input type="checkbox" name="campos[' . stripslashes((trim($v))) . ']">' . $TD2; 
				if($check=='a')$s .=$TD . "<A href=\"$pcheck?id=" . stripslashes((trim($v))) . '">' . stripslashes((trim($v))) . '</A>' . $TD2; 
				continue;
			}
