...
*/
	...
	*/
	...
	/*
		...
		catalogo
	*/
	function GetValue($SQLCAD, $debug = false)
	{
			$this->debug = $debug;
			$_rs =& $this->Execute($SQLCAD);
			$ret = $_rs->fields[0];
			if($_rs) $_rs->close();
			return $ret;
	}
		
	function catalogo($SQLCAD){
		$_rs = $this->Execute($SQLCAD);
		if($_rs){
			$_rs->MoveFirst();
			while (!$_rs->EOF) {
				echo "<option value=\"" . $_rs->fields[0] . "\">" . utf8_encode($_rs->fields[1] ). "</option>\n"; 
				$_rs->MoveNext();
			}
			$_rs->MoveFirst();
		}
	}
	function catalogo2($SQLCAD){
		$_rs = $this->Execute($SQLCAD);
		if($_rs){
			$_rs->MoveFirst();
			while (!$_rs->EOF) {
				echo "<option value=\"" . $_rs->fields[0] . "\">" . ($_rs->fields[1] ). "</option>\n"; 
				$_rs->MoveNext();
			}
			$_rs->MoveFirst();
		}
