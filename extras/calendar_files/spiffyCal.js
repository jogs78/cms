...
img_Del.src="../extras/calendar_files/btn_del_small.gif";
img_Close.src="../extras/calendar_files/btn_close_small.gif";
...
//	this.defaultDateFormat='dd/MMM/yyyy';
	...
	this.matchedFormat="";
	this.DefBtnImgPath='../extras/calendar_files/';
...
'MMM/dd/yyyy'
	//,
//jogs: agregre esta forma
//	'yyyy-M-dd'
	
	);
//

	var MONTH_NAMES = new Array('Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Dicimbre','Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic');
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode, strfmt, isen) {
	var isena = isen ;
	...
	var msDOW       = new makeArray0('D','L','M','Mm','J','V','S');
	...
	// Defaults
	if ( typeof(strfmt) == 'undefined' )
	{
		var strDefDateFmt='dd/M/yyyy';

	}else{
		if (strfmt == '' )	{
			var strDefDateFmt='dd/M/yyyy';	
		}else {
			var strDefDateFmt='yyyy-M-dd';
		}
	}
...
//jogs: aqui esta el loop de los años
			for (intLoop=this.minYearChoice; intLoop<this.maxYearChoice; intLoop++) { 
			//for (intLoop=1975; intLoop<1992; intLoop++) {
	...
	function writeControl() {
		...
		strHold='<input class="cal-TextBox" type="text" name="' + this.textBoxName + '"' + strTemp + 'size="12" value="' + this.getSelectedDate() + '" alt="R,texto,nada,Tu fecha de nacimiento ">';
		if (!scIE) {
