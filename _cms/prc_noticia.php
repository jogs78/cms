...
$sys = new sistema();
$publico = $_SESSION['id'];
if( !$sys->puede($_SESSION['id'] ,NOTICIA, $accion) ) exit; 
$id = $_POST["ide"];
$titulo = $_POST["titulo"]; 
...
if ($accion==3){
    ...
    if ($accion ==1 ){
		$sys->conn->Execute("INSERT INTO noticia VALUES ();");
		$id = $sys->conn->Insert_ID();
		//$accion = 2;
	}
	foreach( $inputs as $file ) {
		if($_FILES[$file]['name']){
			if($up->proccess_file($file ,  $id , "noticia") ){ 
				$$file = $up->lastname;
...
$sys->currentid = $id;
$sys->log($_SESSION['user'] ,NOTICIA, $_POST["act"], $sql);

?>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<title>PROCESO  NOTICIA</title>
<link href="cms.css" rel="stylesheet" type="text/css">
</head>
<body>
<?php
	echo "<h1>PROCESO NOTICIA </h1>";
	$t=0;
	if($e){	
		echo "<hr /><br />$e<br /><hr />";
		$t=10000;
	}else {
		echo "<hr /><br />$msj <br /><hr />"; 
		$t=10000;
	}
?> 
<br />REALIZADO... SERA REDIRECCIONADO A LA LISTA DE ENTRADAS EN SEGUNDOS.... SI NO DE CLICK 
<a HREF="lst_noticia.php">AQUI </a>"; 
<script language="javascript" type="text/javascript">
<!--
	//setTimeout( "document.location='lst_noticia.php'", <?php echo $t ?> );	
-->
</script>
</body>
</html>
