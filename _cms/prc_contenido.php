...
$sys = new sistema();
$accion=$_POST["act"];
if( !$sys->puede($_SESSION['id'] , CONTENIDO, $accion) ) exit;
$id = $_POST["ide"];
$p01 = $_POST["p01"]; 
...
if ($accion==3){
	if ($contenidos_es == 1 )	$sql = "DELETE FROM contenido WHERE id = $id;";
	else $sql = "DELETE FROM libre WHERE id = $id;";
}else {
...
if ($accion==2) {
	...
	if ($contenidos_es == 1 )	$sql="UPDATE contenido SET ";
	else {
...
$sys->currentid = $id;
$sys->log($_SESSION['id'] ,CONTENIDO, $accion, $sql);
//$sys->conn->debug=true;
...
?>
<BR>REALIZADO... SERA REDIRECCIONADO A LA LISTA DE CONTENIDOS EN SEGUNDOS.... SI NO DE CLICK <A HREF="lst_contenido.php">AQUI </A>
